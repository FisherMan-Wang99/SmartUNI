...
def create_weighted_query_embedding(profile, embed_model):
    ...
    academic_parts = []
    academic_parts.append(f"GPA {academics.get('gpa')}")
    academic_parts.append(f"SAT {academics.get('sat')}")
    
    # 新增的考试成绩字段
    if academics.get('act'):
        academic_parts.append(f"ACT {academics.get('act')}")
    if academics.get('toefl'):
        academic_parts.append(f"TOEFL {academics.get('toefl')}")
    if academics.get('ielts'):
        academic_parts.append(f"IELTS {academics.get('ielts')}")
    if academics.get('det'):
        academic_parts.append(f"DET {academics.get('det')}")
    ...
    # 2. 活动背景（替代原来的background）
    activities = profile.get("activities", {})
    activities_parts = []
    
    # 科研经历
    if activities.get("research_months") > 0 and activities.get("research_type"):
        activities_parts.append(f"Research experience: {activities['research_months']} months in {activities['research_type']}")
    
    # 志愿者经历
    if activities.get("volunteer_months") > 0 and activities.get("volunteer_type"):
        activities_parts.append(f"Volunteer experience: {activities['volunteer_months']} months in {activities['volunteer_type']}")
    
    # 实习经历
    if activities.get("internship_months") > 0 and activities.get("internship_type"):
        activities_parts.append(f"Internship: {activities['internship_months']} months in {activities['internship_type']}")
    
    # 艺术经历
    if activities.get("art_months") > 0 and activities.get("art_type"):
        activities_parts.append(f"Art experience: {activities['art_months']} months in {activities['art_type']}")
    
    # 全职工作经历
    if activities.get("full_time_work_months") > 0 and activities.get("full_time_work_type"):
        activities_parts.append(f"Full-time work: {activities['full_time_work_months']} months in {activities['full_time_work_type']}")
    ...
    if preferences.get("size"):
        pref_parts.append(f"Size: {preferences['size']}")
    if preferences.get("major"):
        pref_parts.append(f"Major: {preferences['major']}")
    
    # 新增的偏好字段
    if preferences.get("career_interest"):
        pref_parts.append(f"Career interest: {preferences['career_interest']}")
    if preferences.get("tuition"):
        pref_parts.append(f"Tuition budget: {preferences['tuition']}")
    if preferences.get("teacher_student_ratio"):
        pref_parts.append(f"Teacher-student ratio preference: {preferences['teacher_student_ratio']}")
    if preferences.get("school_atmosphere"):
        pref_parts.append(f"School atmosphere: {preferences['school_atmosphere']}")
    if preferences.get("international_level"):
        pref_parts.append(f"International level: {preferences['international_level']}")
...
# Note: This function is commented out in the main program as climate filtering
#       is now handled through embedding similarity in create_weighted_query_embedding
def filter_by_climate(universities, student_profile):
    # 检查 preferences 中是否包含 climate 字段
    if "climate" not in student_profile.get("preferences", {}):
        return universities  # 如果没有气候偏好，返回所有大学
...
def classify_schools_strict(student, universities):
    ...
    sat = int(student["academics"]["sat"]) if student["academics"]["sat"] else 0
    toefl = int(student["academics"]["toefl"]) if student["academics"]["toefl"] else 0
    ielts = float(student["academics"]["ielts"]) if student["academics"]["ielts"] else 0
    det = int(student["academics"]["det"]) if student["academics"]["det"] else 0
    act = int(student["academics"]["act"]) if student["academics"]["act"] else 0
    
    # 计算学生综合分数（考虑多种考试成绩，使用最高的分数作为代表）
    # 语言成绩转换和标准化：
    # TOEFL: 0-120, IELTS: 0-9 (转换为TOEFL分数: IELTS * 13.33), DET: 0-160 (转换为TOEFL分数: DET * 0.75)
    # SAT和ACT转换：ACT = SAT/40 (近似值)
    
    # 语言成绩标准化
    language_score = toefl
    if ielts > 0:
        language_score = max(language_score, int(ielts * 13.33))
    if det > 0:
        language_score = max(language_score, int(det * 0.75))
    
    # 标准化考试成绩
    test_score = sat
    if act > 0:
        test_score = max(test_score, act * 40)  # ACT转SAT近似值
    
    # 计算综合分数
    student_score = (gpa * 500) + (test_score * 0.5) + (language_score * 2)  # 权重调整使各因素平衡
...
def create_student_query(profile):
    ...
    if ac.get('act'):
        academic_parts.append(f"ACT {ac['act']}")
    if ac.get('toefl'):
        academic_parts.append(f"TOEFL {ac['toefl']}")
    if ac.get('ielts'):
        academic_parts.append(f"IELTS {ac['ielts']}")
    if ac.get('det'):
        academic_parts.append(f"DET {ac['det']}")
    ...
    # 科研经历
    if activities.get('research_months') > 0 and activities.get('research_type'):
        activities_parts.append(f"{activities['research_months']} months research in {activities['research_type']}")
    
    # 志愿者经历
    if activities.get('volunteer_months') > 0 and activities.get('volunteer_type'):
        activities_parts.append(f"{activities['volunteer_months']} months volunteer in {activities['volunteer_type']}")
    
    # 实习经历
    if activities.get('internship_months') > 0 and activities.get('internship_type'):
        activities_parts.append(f"{activities['internship_months']} months internship in {activities['internship_type']}")
    
    # 其他经历
    if activities.get('art_months') > 0 and activities.get('art_type'):
        activities_parts.append(f"{activities['art_months']} months art in {activities['art_type']}")
    if activities.get('full_time_work_months') > 0 and activities.get('full_time_work_type'):
        activities_parts.append(f"{activities['full_time_work_months']} months full-time work in {activities['full_time_work_type']}")
    ...
    # 新增的偏好字段
    if pref.get('career_interest'):
        pref_parts.append(f"career interest in {pref['career_interest']}")
    if pref.get('tuition'):
        pref_parts.append(f"tuition budget {pref['tuition']}")
    if pref.get('teacher_student_ratio'):
        pref_parts.append(f"{pref['teacher_student_ratio']} teacher-student ratio")
    if pref.get('school_atmosphere'):
        pref_parts.append(f"{pref['school_atmosphere']} school atmosphere")
    if pref.get('international_level'):
        pref_parts.append(f"{pref['international_level']} international level")
...
"""
    ...
    for cat_name, uni_list in categories.items():
        ...
        for i, uni in enumerate(uni_list):
            ...
            admission_threshold = json.loads(uni.get('admission_threshold', '{}'))
            
            # 计算录取概率（基于分数差距）
            if student_profile and admission_threshold:
                try:
                    student_gpa = float(student_profile['academics']['gpa'])
                    student_sat = int(student_profile['academics']['sat']) if student_profile['academics']['sat'] else 0
                    
                    uni_gpa_req = admission_threshold.get('gpa', 0)
                    uni_sat_req = admission_threshold.get('sat', 0)
                    
                    # 简单计算录取概率（仅供参考）
                    gpa_diff = student_gpa - uni_gpa_req
                    sat_diff = student_sat - uni_sat_req
                    
                    if gpa_diff >= 0.3 and sat_diff >= 100:
                        admission_chance = "高"
                    elif gpa_diff >= 0 and sat_diff >= 0:
                        admission_chance = "中等"
                    else:
                        admission_chance = "较低"
                except:
            ...
            formatted_text += (
                ...
                f"- **规模**: {uni.get('size')}\n"
                f"- **气候**: {uni.get('climate')}\n"
                f"- **优势项目**: {', '.join(academics.get('strong_programs', []))}\n"
                f"- **学校文化**: {uni.get('culture')}\n"
